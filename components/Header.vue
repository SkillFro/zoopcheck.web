<template>
  ...
  <div class="hidden lg:block">
    <div
      ...
      <div class="xl:max-w-[1120px] w-full mx-auto flex items-center justify-between ">
        <a href="/">
        ...
        <div class="flex items-center xl:gap-[64px] gap-[30px]">
          <div class="flex items-center xl:gap-[16px] gap-[10px]">
            <div v-for="nav in navs" :key="nav"
              ...
              :class="getRoute(nav.path) ? '  text-[#086BD8] underline underline-offset-[12px] decoration-[#086BD8] decoration-2' : 'text-gray-800'
                ">
              ...
              <div v-if="showOptions"
                ...
                <div class="py-1 md:px-1 space-y-0.5">
                  <a class="p-3 flex gap-x-4 hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 rounded-lg "
                    ...
                    <div class="grow">
                      <span class="block font-semibold text-sm text-gray-800 ">Candidates</span>
                      <p class="text-sm text-gray-500 ">How we connect you with your perfect career opportunity</p>
                    </div>
                  </a>
                  ...
                  <a class="p-3 flex gap-x-4 hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 rounded-lg "
                    ...
                    <div class="grow">
                      <span class="block font-semibold text-sm text-gray-800 ">Recruiters</span>
                      <p class="text-sm text-gray-500 ">How we help you find and hire top talent faster</p>
                    </div>
                  </a>
                </div>
              </div>
          ...
          <div v-else class="relative cursor-pointer">
            ...
            <img @click="() => toggle = !toggle" class="w-10 h-10 rounded-full"
              :src="authState.data.user.profile !== '' && authState.data.user.profile !== undefined ? authState.data.user.profile : 'https://img.freepik.com/premium-vector/default-avatar-profile-icon-social-media-user-image-gray-avatar-icon-blank-profile-silhouette-vector-illustration_561158-3383.jpg'"
              alt="">
            <div v-if="toggle"
              class="bg-white shadow text-[16px] flex flex-col absolute z-10 right-2 p-2 top-10 gap-2 items-end mt-2">
              <NuxtLink to="/profile?tab=profile" @click="() => toggle = !toggle" class="cursor-pointer px-4">Account</NuxtLink>

              <NuxtLink @click="logout" class="text-red-500 px-4 cursor-pointer">Log Out</NuxtLink>
            </div>
          </div>
  ...
  <div class="fixed top-0 z-10 block w-full py-4 bg-white lg:hidden">
    <div class="flex items-center justify-between px-4 lg:px-10">
      ...
      <button @click="toggleNav()"
        ...
        <span><img src="/public/images/bar.svg" width="30px" height="30px" alt="" /></span>
      </button>
    </div>
    <div v-if="nav" class="w-full h-screen  bg-black/70">
      <div class="bg-white p-10">
        <div class="flex flex-col items-end justify-center gap-[40px]">
          ...
          <div class="flex flex-col gap-[16px]">
            <button class="py-[6px] px-[12px] bg-[#086BD8] text-white text-[16px] rounded-[0.375rem]">
              Sign In
            </button>
            <button class="py-[6px] px-[12px] bg-[#0a2d52] text-white text-[16px] rounded-[0.375rem]">
              Get Started
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getRoute(path) {
      return this.$route.path === path || this.$route.fullPath.replace('/', '').split('/').includes(path.replace('/', ''))
    },
    toggleOptions() {
