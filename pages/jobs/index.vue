  ...
  <div class="w-full max-w-5xl mx-auto mt-32 px-4 overflow-y-auto scroll-hide" style="max-height: calc(100vh - 150px)" ref="scrollContainer">
    <div class="w-full my-5">
      <div class="relative flex items-center w-full">
        ...
        <input type="text" v-model="search" placeholder="Search jobs, locations..."
    ...
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <div class="w-full lg:w-2/3">
        <div v-if="jobs !== null" class="flex flex-col w-full gap-5">
          <div v-for="job in jobs" :key="job.id" class="p-4 bg-white border border-slate-200 rounded-xl">
        ...
        <div v-else class="flex h-screen justify-center lg:w-[1080px] items-center">
          <div class="flex flex-col h-1/2 items-center">
            <div class="w-8 h-8 animate-spin border-[#086BD8] border-3 rounded-full border-r-gray-400"></div>
            <p class="text-sm md:text-lg mt-4 text-center">We're getting the job details for you</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="jobs !== null" class="hidden lg:block w-full md:w-1/3">
        <div class="p-4 bg-white border border-slate-200 rounded-xl sticky top-32">
            ...
            <div class="space-y-2">
              <label class="flex items-center">
                <input type="checkbox" v-model="filters.workMode" value="Remote"
                  class="rounded text-[#086BD8] focus:ring-[#086BD8]" @change="handleFilterChange">
              ...
              <label class="flex items-center">
                <input type="checkbox" v-model="filters.workMode" value="On-site"
                  class="rounded text-[#086BD8] focus:ring-[#086BD8]" @change="handleFilterChange">
          ...
          <div class="mb-6">
            ...
            <div v-if="locationSuggestions.length" class="mt-2 max-h-48 overflow-y-auto border rounded-md">
          ...
          <button @click="resetFilters" class="w-full py-2 bg-gray-100 hover:bg-gray-200 rounded-md text-sm">
            Reset Filters
          </button>
        </div>
      </div>
  ...
  methods: {
    ...
    async handleLocationSearch() {
      if (this.locationSearch.length > 1) {
        try {
          const response = await this.$apiFetch(`/locations?search=${this.locationSearch}`);
    ...
    async fetchJobs(initialLoad = false) {
      ...
      try {
        ...
        if (this.filters.workMode.length) params.append('workMode', this.filters.workMode.join(','));
        ...
        if (this.filters.datePosted) params.append('datePosted', this.filters.datePosted);
        ...
        this.noMoreJobs = response.posts?.length < this.limit ||
