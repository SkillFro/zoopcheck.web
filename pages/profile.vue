<template>
  <div
    class="md:max-w-[1080px] max-w-full md:mx-auto md:px-4 lg:px-0 md:mt-32 mt-24"
  >
    <div class="md:flex md:flex-row flex-col gap-20 justify-between">
      <div class="md:sticky md:top-20 z-10">
        <div
          class="flex md:sticky md:top-32 flex-col md:w-[250px] w-full bg-[#f8f9fa] md:bg-none pb-10 px-4"
        >
      ...
      <div class="flex-1 p-2">
        <section v-if="selectedTab === 'profile'" class="flex-1">
          ...
          <div v-if="user !== null" class="flex px-4 gap-6">
            <!-- <div>
              ...
              <div class="pb-12">
                <div class="flex justify-between items-start">
                  <div>
                    ...
                    <p class="mt-1 text-sm/6 text-gray-600 w-[250px] md:w-full">
                  ...
                  >
                    <img
                      ...
                      class="w-5 h-5"
                    />
                    <p class="text-[#1447e6] text-sm">Edit</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div class="sm:col-span-4">
                    <label
                      for="email"
                      class="block text-sm/6 font-medium text-gray-900"
                      ><span v-if="user.role === 'recruiter'"
                        >Company website</span
                      ><span v-if="user.role === 'candidate'"
                        >Portfolio website or LinkedIn Profile</span
                      ></label
                    >
                    <div class="mt-4">
                      <input
        ...
        <section v-if="selectedTab === 'jobs'" class="flex-1 pb-10">
                ...
                <div class="flex flex-col w-full gap-2 lg:gap-5">
                  ...
                  >
                    ...
                    <textarea
                      ...
                      class="rounded-md border-slate-200 border p-3 text-[16px] placeholder:text-[#555a64] outline-none w-full h-[100px]"
                    ></textarea>
                  </div>
                  <div
                    class="flex items-start flex-col gap-[10px] w-full md:w-auto"
                  ...
                  >
                    ...
                    <div
                      ...
                      class="mt-2 max-h-48 w-full overflow-y-auto border rounded-md"
                    >
                      <div
          ...
          >
                ...
                >
                  ...
                  <button
                    ...
                    @click="
                      () => {
                        useRouter().push(
                          `/profile?tab=applications&id=${job.id}`
                        );
                      }
                    "
                  >
            ...
            <div v-else class="col-span-2 w-full">
              <p class="text-lg mt-4 text-center">No Job Post Found !</p>
            </div>
          </div>
        ...
        <section
          v-if="
            ...
            useAuth().data.value.user.role === 'recruiter'
          "
              ...
              <div class="flex flex-col gap-3 p-2">
                ...
                <div class="flex flex-col mt-5 w-full gap-2 lg:gap-5">
                  ...
                  >
                    ...
                    >
                    <select
                      v-model="applicationDetails.jobStatus"
                      class="rounded-md border-slate-200 border p-3 text-[16px] placeholder:text-[#555a64] outline-none w-full"
                    >
                  ...
                  >
                    ...
                    >
                    <select
                      v-model="applicationDetails.noticePeriod"
                      class="rounded-md border-slate-200 border p-3 text-[16px] placeholder:text-[#555a64] outline-none w-full"
                    >
                  ...
                  >
                    ...
                    >
                    <input
                      v-model="applicationDetails.salary"
                      type="number"
                      name="experience"
                      id="experience"
        ...
        <section
          v-if="
            selectedTab === 'applications' &&
            useAuth().data.value.user.role === 'candidate'
          "
          class="flex-1 h-screen"
        >
          <div
          ...
          >
            <div class="flex gap-40">
              <div class="flex gap-10">
                <img
                  :src="application.recruiter.profile"
                  alt="profile"
                  class="w-10 h-10 rounded-full bg-stone-100"
                />
                <div class="">
            ...
            <div class="flex flex-col gap-2 items-end">
              <a
                ...
                class="text-[#086BD8] text-sm"
              >
                View Job
              </a>
              <p class="text-xs">
                Recruiter seen :
                {{ application.viewed ? application.viewedAt : "Yet" }}
              </p>
            </div>
          </div>
        </section>
    ...
    >
      ...
      <div
        class="bg-white rounded-lg shadow-lg p-8 z-10 max-w-lg mx-auto overflow-auto max-h-full"
      >
    ...
    >
      ...
      >
        ...
        <div class="sm:col-span-4">
          <label for="email" class="block text-sm/6 font-medium text-gray-900"
            ><span v-if="user.role === 'recruiter'">Company website</span
            ><span v-if="user.role === 'candidate'"
              >Portfolio website or LinkedIn Profile</span
            ></label
          >
        ...
        <div class="flex items-center gap-5 justify-end w-full">
          <button
          ...
          <button
            @click="updateProfile()"
            class="py-2 px-4 rounded bg-[#086BD8] text-white w-fit cursor-pointer"
            :disabled="loading"
          >
            <span v-if="loading">Processing...</span>
  ...
  methods: {
    ...
    openApplicationDetails(value, data) {
      ...
      if (value) {
        ...
        if (data.viewedAt === null) {
          this.$apiFetch(`/recruiter/application/${data.id}`, {
            method: "PUT",
          });
        }
      } else {
    ...
    async getUSerInfo() {
      const response = await this.$apiFetch(
        `/${useAuth().data.value.user.role}/me`
      );
    ...
    async getApplications() {
      const response = await this.$apiFetch(
        useAuth().data.value.user.role === "recruiter"
          ? `/recruiter/post/${useRoute().query.id}/applications`
    ...
    async getCategories() {
      const response = await this.$apiFetch(`/categories`);
    ...
    async handleLocationSearch() {
      if (this.formData.location.length > 1) {
        try {
          const response = await this.$apiFetch(
            `/locations?search=${this.formData.location}`
          );
    ...
    async createJob() {
      ...
      this.loading = true;
      const response = await this.$apiFetch(`/recruiter/post`, {
        method: "POST",
        body: this.formData,
      }).catch((err) => {
        push.error({ title: "Error", message: response.message });
