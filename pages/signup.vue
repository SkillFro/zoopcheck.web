<template>
  <div class="flex min-h-screen flex-col justify-center px-6 py-12 lg:px-8">
    ...
    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
      <div class="space-y-6">
        <div>
          <label for="name" class="block text-sm/6 font-medium text-gray-900"><span v-if="role==='recruiter'">Company</span> Name</label>
          <div class="mt-2">
      ...
      <p class="mt-10 text-center text-sm/6 text-gray-500">
        Already a member?
        <a href="/signin" class="font-semibold text-[#086BD8] hover:text-[#086BD8]">Sign In</a>
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async signUp() {
      this.loading = true;
      await useAuth().signUp({ name: this.name, email: this.email, password: this.password,role:this.role }, { callbackUrl: '/signin' ,redirect:true }).then((res) => {
